...
class ApplymaticAI:
    ...
    def generate_cover_letter(self, resume_text):
        ...
        CRITICAL RULES:
        1. Use the exact text "{{company_name}}" as a placeholder for the target company (do not invent a company).
    ...
    def refine_cover_letter(self, current_text):
        prompt = f"""
        ...
        CRITICAL RULES:
        1. Keep the "{{company_name}}" placeholder intact if it exists.
        2. Do not add unnecessary addresses or dates at the top.
        3. only send response, don't talk about anything else or saying Here's a refined and polished version of the cover letter or any thing like that
